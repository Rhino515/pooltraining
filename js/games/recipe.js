...
export function recipeFields(ch) {
  ...
  return {
    ...
    english: englishText(
      ch.english?.hTips || 0,
      ch.english?.type && ch.english.type !== 'none'
        ? ch.english.type
        : null
    ),
...
/**
 * Compact strip:
 * mini ball + SPEED chip + OB contact + route chip
 */
export function recipeStripHTML(
  ch,
  { hideAim = false, hideRoute = false, hidden = false } = {}
) {
  if (hidden) {
    return `<button type="button" class="recipeStrip hiddenRecipe" data-action="noop">
      <span class="rs-q">?</span>
      <span>Recipe hidden — plan your shot below</span>
    </button>`;
  }
    ...
    <span class="rs-chips">
      ...
      ${
        hideAim || !ch.aim?.short
          ? ''
          : `<span class="rs-chip">${esc(ch.aim.short)}</span>`
      }

      ${
        hideRoute
          ? ''
          : `<span class="rs-chip">${esc(ch.route?.short || '')}</span>`
      }

      ${
        ch.english?.hTips
          ? `<span class="rs-chip eng">${esc(englishText(ch.english.hTips))}</span>`
          : ''
      }
    </span>
...
export function tipLabel(vTips = 0, hTips = 0) {
  ...
  const tw = (x) =>
    `${fracTips(x)} tip${Math.abs(x) > 1 ? 's' : ''}`;

  if (!v && !h) return 'Center';

  if (!h) {
    return `${v > 0 ? 'Top' : 'Draw'} ${tw(v)}`;
  }
...
/**
 * Cue-ball clock description.
 *
 * Examples:
 * Top / center-high  -> 12:00
 * High Right        -> 1:30
 * Right             -> 3:00
 * Low Right         -> 4:30
 * Bottom / draw     -> 6:00
...
export function tipClockLabel(vTips = 0, hTips = 0) {
  ...
  const angle = Math.atan2(h, v);

  let hours = (angle * 6 / Math.PI + 12) % 12;
...
export function tipGaugeSVG(contact) {
  ...
  s += `<defs>
    <radialGradient id="cw${uid}" cx="38%" cy="32%" r="78%">
      <stop offset="0" stop-color="#ffffff"/>
      <stop offset=".55" stop-color="#e8eff4"/>
      <stop offset=".85" stop-color="#b6c5cf"/>
...
export function speedAngle(speed) {
  const v = Math.max(
    SPEED_MIN,
    Math.min(SPEED_MAX, Number(speed) || SPEED_MIN)
  );

  return -135 +
    ((v - SPEED_MIN) / (SPEED_MAX - SPEED_MIN)) * 270;
}

const polar = (deg, rad) => ({
  x: 50 + rad * Math.sin((deg * Math.PI) / 180),
  y: 50 - rad * Math.cos((deg * Math.PI) / 180)
});
...
function arcPath(a0, a1, rad) {
  ...
  return `M${r2(p0.x)} ${r2(p0.y)} A${rad} ${rad} 0 ${
    a1 - a0 > 180 ? 1 : 0
  } 1 ${r2(p1.x)} ${r2(p1.y)}`;
}
...
export function speedDialSVG(speed) {
  ...
  let s = `<svg
    ...
    viewBox="0 0 100 100"
    role="img"
    aria-label="${esc(speedLabel(speed))}"
    data-angle="${r2(a)}"
  >`;

  s += `<defs>
    <radialGradient id="sdf${++gUid}" cx="50%" cy="38%" r="70%">
      <stop offset="0" stop-color="#eef6fa"/>
      <stop offset=".8" stop-color="#c9d7df"/>
      <stop offset="1" stop-color="#8ea3b1"/>
    </radialGradient>
  </defs>`;
...
/**
 * Shot Recipe gauge card:
 * Aim View · Cue-ball tip · Speed dial.
 *
 * hideAim:
 * coaching hides the aim gauge.
 *
 * No verifiable aim:
 * lag / no OB means aim gauge is omitted.
 */
export function recipeGaugesHTML(
  ch,
  { hideAim = false, hideRoute = false } = {}
) {
  const cc = ch.cueContact || {
    vTips: 0,
    hTips: 0
  };
  ...
  if (info && hideAim) {
    cells.push(
      `<button
        type="button"
        class="gauge gauge-aim hiddenAim"
        data-action="noop"
        data-aim="hidden"
      >
        ${aimViewSVG(null, { hidden: true })}
        <b>Your aim</b>
        <small>plan it</small>
      </button>`
    );
    ...
    cells.push(
      `<button
        type="button"
        class="gauge gauge-aim"
        data-action="why-open"
        data-aim="${esc(info.label)}"
        data-cut="${info.deg}"
        data-side="${info.side}"
        aria-label="${esc(info.plain)}"
      >
        ${aimViewSVG(info)}
        <b>${esc(info.label)}</b>
        <small>${info.deg}° cut</small>
      </button>`
    );
  }
  ...
  cells.push(
    `<button
      type="button"
      class="gauge gauge-tip"
      data-action="recipe-open"
      aria-label="Cue-ball tip ${esc(
        contactText(cc.vTips, cc.hTips)
      )}, ${esc(
        tipClockLabel(cc.vTips, cc.hTips)
      )}"
    >
      ${tipGaugeSVG(cc)}
  ...
  cells.push(
    `<button
      ...
      data-action="recipe-open"
      data-speed="${formatSpeed(ch.speed)}"
      aria-label="${esc(speedLabel(ch.speed))}"
    >
      ${speedDialSVG(ch.speed)}

      <b>
        Speed ${formatSpeed(ch.speed)}
      </b>
  ...
  return `<div
    class="gaugeCard n${cells.length}"
    data-gauges="${cells.length}"
  >${cells.join('')}</div>`;
}
...
export function aimRowHTML(ch) {
  ...
  return `<div
    ...
    data-side="${info.side}"
  >
    ${aimViewSVG(info, { size: 'lg' })}

    <div>
      <span>
        AIM VIEW · looking down the cue
      </span>

      <b>
        ${esc(info.label)} · ${info.deg}° cut
      </b>

      <small>
        ${esc(info.plain)}.
        The dashed ball is where your cue ball must be at contact;
        cover ${Math.round(info.fullness * 100)}% of the ${info.ob.n}.
        ${
          info.afterRail
            ? ' Measured on the last leg after the rail.'
            : ''
        }
      </small>
    </div>
...
export function setupLineHTML(ch) {
    ...
    .map(
      (b) =>
        `<span
          class="su-ball"
          data-ball="${esc(b.id)}"
          data-head="${b.fromHead}"
          data-top="${b.fromTop}"
        >
          <i
            class="su-dot${b.id === 'cue' ? ' cue' : ''}"
            style="--c:${
              b.id === 'cue'
                ? '#f5f7fa'
                : ballColor(b.id)
  ...
  return `<button
    type="button"
    class="setupLine"
    data-action="setup-open"
    aria-label="Ball setup in diamonds"
  >
    <span class="su-label">
      <b>SETUP</b>
      <small>head · top ⓘ</small>
    </span>
...
/**
 * Setup sheet body:
 * convention in plain words + every ball
 */
export function setupSheetHTML(ch) {
  const balls = setupBalls(ch);

  return `<div class="setupSheet">
...
const ballColor = (n) =>
  BALL_COLORS[Number(n)] || '#94a3b8';

const WHY_TITLES = {
  whyCustom: "Coach's note",
  whyContact: 'Why this cue-ball contact',
  whySpeed: 'Why this speed',
  whySpin: 'Why this spin',
...
export function whyHTML(ch) {
  ...
  const keys = Object.keys(WHY_TITLES)
    .filter((k) => w[k]);

  if (!keys.length) {
    return '<p class="muted">No explanation for this layout.</p>';
  }
